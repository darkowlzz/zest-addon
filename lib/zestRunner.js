...
let lastResponse, lastRequest, stmtCount = 0;;

function run(script, worker) {
  let statements = script.getStatements();

  for (let stmt of statements) {
    stmtCount++;
    runStatement(script, stmt, worker);
  }
  stmtCount = 0;
}
exports.run = run;

function runStatement(script, stmt, worker) {
  lastRequest = stmt;
  let opts, reqVar, resVar, runResult;
  switch (stmt.elementType) {
    case 'ZestRequest':
      ...
      reqVar = setStandardReqVariables(lastRequest);
      resVar = setStandardResVariables(lastResponse.resp);
      resVar.set('response.url', stmt.url);
      runResult = {
        result: handleResponse(reqVar, resVar),
        stmtId: stmtCount
      }
...
function setStandardResVariables(response) {
  let resMap = new Map();
  resMap.set('status', response.status);
