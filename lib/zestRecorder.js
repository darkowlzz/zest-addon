...
function ZestRecorder(worker) {
  ...
  this.sidebarWorker.port.on('CHANGE_ATTR', (node) => {
    ...
    let stmt = z.statements[target];
    let i;
    for (i = 0; i < stmt.assertions.length; i++) {
      if (stmt.assertions[i].rootExpression.elementType == node.attr) {
        switch(node.attr) {
          case 'ZestExpressionStatusCode':
            stmt.assertions[i].rootExpression.code = node.value;
            break;
          case 'ZestExpressionLength':
            stmt.assertions[i].rootExpression.approx = node.value;
            break;
          default:
        }
      }
