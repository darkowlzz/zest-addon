...
function ZestRecorder(worker) {
  ...
  this.sidebarWorker.port.on('CHANGE_ATTR', (node) => {
    ...
    if (node.changes.type == 'ZestRequest') {
      ...
      stmts[target].url = changes.requestURL;
      stmts[target].method = changes.requestMethod;
      stmts[target].data = changes.requestBody;
      stmts[target].headers = changes.requestHeader;
      stmts[target].response.statusCode = changes.responseStatusCode;
      stmts[target].response.responseTimeInMs = changes.responseTime;
      stmts[target].response.headers = changes.responseHeader;
      stmts[target].response.body = changes.responseBody;
    }
    else {
      for (let i = 0; i < stmts[target].assertions.length; i++) {
        if (stmts[target].assertions[i].rootExpression.elementType == node.changes.type) {
          switch(node.changes.type) {
            case 'ZestExpressionStatusCode':
              stmts[target].assertions[i].rootExpression.code = node.changes.code;
              break;
            case 'ZestExpressionLength':
              stmts[target].assertions[i].rootExpression.variableName = node.changes.variableName;
              stmts[target].assertions[i].rootExpression.length = node.changes.length;
              stmts[target].assertions[i].rootExpression.approx = node.changes.approx;
              break;
            default:
          }
          break;
